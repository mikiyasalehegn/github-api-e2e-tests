...
def create_temporary_repo(request):
    ...
    repo_name = f"tmp-repo-{int(time.time())}"
    client = GitHubClient(UserTestData.base_url, UserTestData.token)
    repo_api = RepositoryApi(client)
    payload = {"name": repo_name, "description": "Temp repo for E2E tests", "private": False}
    ...
    if request.cls is not None:
        request.cls.repo_name = repo_name

    yield repo_name  # The tests run here

    # TEARDOWN: Delete the repo after the class finishes
    repo_api.delete_repo(owner=user_name, repo_name=repo_name)
