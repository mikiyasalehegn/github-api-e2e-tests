...
class TestRepoCollaboratorHandshakeFlow(BaseTest):
    ...
    def test_repo_collaborator_handshake_flow(self, create_temporary_repo):
        ...
        # repo_collab_client = self.get_client(token=COLLABORATOR_TOKEN)
        # add_repo_collab_api = RepoCollabApi(repo_collab_client)
        ...
        # -------------------- Check if a user is repo collaborator --------------------

        check_repo_collab_resp = self.repo_collab_api.check_repo_collaborator(owner=USERNAME, repo=collab_temp_repo,
                                                                              username=COLLABORATOR)
        ...
        remove_repo_collab_response = self.repo_collab_api.remove_repo_collaborator(owner=USERNAME,
                                                                                    repo=collab_temp_repo,
                                                                                    username=COLLABORATOR)
        ...
        check_repo_collab_resp = self.repo_collab_api.check_repo_collaborator(owner=USERNAME, repo=collab_temp_repo,
                                                                              username=COLLABORATOR)
        assert check_repo_collab_resp.status_code == 404
